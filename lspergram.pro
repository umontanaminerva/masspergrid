PRO lspergram,tim,data,nu_out,peri_out,pkperiods,pkheights,title=title,     $
            ...
            noise=noise, numf = numf_in, outfap = faps, plot = plot
;+
; NAME:
;         lspergram
;
...
;-
   ...
   IF (n_elements(fmax) EQ 0) THEN BEGIN 
       IF (n_elements(pmin) EQ 0) THEN BEGIN 
           pmin = 0.5   ; Default shortest period ( 2 days )
...
dnu = 1./(4.*timlen)
numf = long((fmax - fmin)/dnu+1)  ;Number of sampled frequencies.
...
;       st_faps = strmid(strtrim(string(faps*100.),2),0,3) ;false alarm p
       ...
       if keyword_set(plot) then plot, 1./nu_out,  peri_out, /xl, xtit = 'Period', ytit = 'Power'
